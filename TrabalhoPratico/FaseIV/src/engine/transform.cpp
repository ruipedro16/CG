...
Transform::Transform(const std::string &type, float angle, float x, float y, float z)
...
void Transform::setX(float x) { this->x = x; }

float Transform::getY() const { return y; }

void Transform::setY(float y) { this->y = y; }

float Transform::getZ() const { return z; }

void Transform::setZ(float z) { this->z = z; }

float Transform::getTime() const { return time; }

const std::vector<Vertex *> &Transform::getPointsCurve() const { return pointsCurve; }
...
void Transform::getCatmullRomPoint(float t, int *indexes, float *p, float *der) {
...
void Transform::setCatmullPoints() {
    ...
    for (float i = 0; i < 1; i += 0.01) {
        getGlobalCatmullRomPoint(i, point, der);
        Vertex *p = new Vertex(point[0], point[1], point[2]);
