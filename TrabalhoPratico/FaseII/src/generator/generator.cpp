...
vector<Vertex> generate_box(float width, float height, float length, int divisions) {
    vector<Vertex> coords;
...
vector<Vertex> generate_torus(float inner_radius, float outer_radius, int slices, int stacks) {
    ...
    for (int i = 0; i < slices; i++, theta += theta_shift) {
        for (int j = 0; j < stacks; j++, phi += phi_shift) {
            coords.push_back(Vertex(cos(theta) * (outer_radius + inner_radius * cos(phi)),
                                    sin(theta) * (outer_radius + inner_radius * cos(phi)),
                                    inner_radius * sin(phi)));
            coords.push_back(
                Vertex(cos(theta + theta_shift) * (outer_radius + inner_radius * cos(phi)),
                       sin(theta + theta_shift) * (outer_radius + inner_radius * cos(phi)),
                       inner_radius * sin(phi)));
            coords.push_back(Vertex(
                cos(theta + theta_shift) * (outer_radius + inner_radius * cos(phi + phi_shift)),
                sin(theta + theta_shift) * (outer_radius + inner_radius * cos(phi + phi_shift)),
            ...
            coords.push_back(Vertex(
                ...
                inner_radius * sin(phi + phi_shift)));
            coords.push_back(
                Vertex(cos(theta) * (outer_radius + inner_radius * cos(phi + phi_shift)),
...
int main(int argc, char **argv) {
    ...
    if (!strcmp(argv[1], "plane")) {
        ...
        if (argc != 6) {
            cout << "Inavalid number of arguments to create sphere" << endl;
            return 1;
        } else
            vertices = generate_sphere(stof(argv[2]), stoi(argv[3]), stoi(argv[4]));
    } else if (!strcmp(argv[1], "cone")) {
        if (argc != 7) {
            cout << "Inavalid number of arguments to create cone" << endl;
            return 1;
        } else
            vertices = generate_cone(stof(argv[2]), stof(argv[3]), stoi(argv[4]), stoi(argv[5]));
    } else if (!strcmp(argv[1], "torus")) {
        if (argc != 7) {
            cout << "Inavalid number of arguments to create torus" << endl;
            return 1;
        } else
